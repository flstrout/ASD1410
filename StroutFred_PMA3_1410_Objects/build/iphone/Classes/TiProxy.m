...
//Rare exceptions to indicate a bug in the _stroutfred_pma3_1410_objects code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_stroutfred_pma3_1410_objects = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_stroutfred_pma3_1410_objects]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.stroutfred_pma3_1410_objects.module"
