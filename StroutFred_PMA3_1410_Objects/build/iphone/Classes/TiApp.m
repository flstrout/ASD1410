...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kStroutFred_PMA3_1410_ObjectsUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kStroutFred_PMA3_1410_ObjectsUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a StroutFred_PMA3_1410_Objects app using the node.js CLI
