...
NSString * StroutFred_PMA3_1410_Objects$ModuleRequireFormat = @"(function(exports){"
...
@implementation StroutFred_PMA3_1410_ObjectsObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_stroutfred_pma3_1410_objects);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_stroutfred_pma3_1410_objects gc];
}
...
{
	// create StroutFred_PMA3_1410_Objects global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "StroutFred_PMA3_1410_Objects" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_stroutfred_pma3_1410_objects = [[StroutFred_PMA3_1410_ObjectsObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_stroutfred_pma3_1410_objects];
	
	NSString *_stroutfred_pma3_1410_objectsNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _stroutfred_pma3_1410_objectsNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_stroutfred_pma3_1410_objects valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:StroutFred_PMA3_1410_Objects$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.stroutfred_pma3_1410_objects.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.stroutfred_pma3_1410_objects.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
