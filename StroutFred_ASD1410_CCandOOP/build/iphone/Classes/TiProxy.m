...
//Rare exceptions to indicate a bug in the _stroutfred_asd1410_ccandoop code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_stroutfred_asd1410_ccandoop = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_stroutfred_asd1410_ccandoop]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.stroutfred_asd1410_ccandoop.module"
