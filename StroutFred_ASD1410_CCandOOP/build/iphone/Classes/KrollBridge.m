...
NSString * StroutFred_ASD1410_CCandOOP$ModuleRequireFormat = @"(function(exports){"
...
@implementation StroutFred_ASD1410_CCandOOPObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_stroutfred_asd1410_ccandoop);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_stroutfred_asd1410_ccandoop gc];
}
...
{
	// create StroutFred_ASD1410_CCandOOP global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "StroutFred_ASD1410_CCandOOP" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_stroutfred_asd1410_ccandoop = [[StroutFred_ASD1410_CCandOOPObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_stroutfred_asd1410_ccandoop];
	
	NSString *_stroutfred_asd1410_ccandoopNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _stroutfred_asd1410_ccandoopNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_stroutfred_asd1410_ccandoop valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:StroutFred_ASD1410_CCandOOP$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.stroutfred_asd1410_ccandoop.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.stroutfred_asd1410_ccandoop.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
