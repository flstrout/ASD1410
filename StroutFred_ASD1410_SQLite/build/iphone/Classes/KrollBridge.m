...
NSString * StroutFred_ASD1410_SQLite$ModuleRequireFormat = @"(function(exports){"
...
@implementation StroutFred_ASD1410_SQLiteObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_stroutfred_asd1410_sqlite);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create StroutFred_ASD1410_SQLite global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "StroutFred_ASD1410_SQLite" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_stroutfred_asd1410_sqlite = [[StroutFred_ASD1410_SQLiteObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_stroutfred_asd1410_sqlite];
	
	NSString *_stroutfred_asd1410_sqliteNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _stroutfred_asd1410_sqliteNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_stroutfred_asd1410_sqlite valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:StroutFred_ASD1410_SQLite$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.stroutfred_asd1410_sqlite.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.stroutfred_asd1410_sqlite.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
