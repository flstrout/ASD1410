...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kStroutFred_ASD1410_SQLiteUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kStroutFred_ASD1410_SQLiteUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a StroutFred_ASD1410_SQLite app using the node.js CLI
