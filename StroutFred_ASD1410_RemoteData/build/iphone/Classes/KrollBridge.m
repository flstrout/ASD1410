...
NSString * StroutFred_ASD1410_RemoteData$ModuleRequireFormat = @"(function(exports){"
...
@implementation StroutFred_ASD1410_RemoteDataObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_stroutfred_asd1410_remotedata);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_stroutfred_asd1410_remotedata gc];
}
...
{
	// create StroutFred_ASD1410_RemoteData global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "StroutFred_ASD1410_RemoteData" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_stroutfred_asd1410_remotedata = [[StroutFred_ASD1410_RemoteDataObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_stroutfred_asd1410_remotedata];
	
	NSString *_stroutfred_asd1410_remotedataNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _stroutfred_asd1410_remotedataNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_stroutfred_asd1410_remotedata valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:StroutFred_ASD1410_RemoteData$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.stroutfred_asd1410_remotedata.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.stroutfred_asd1410_remotedata.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
